...
<template>
  ...
  <a-form class="smart-query-form">
    <a-row class="smart-query-form-row">
      ...
      >
        <DictSelect
            :dict-code="DICT_CODE_ENUM.FILE_TYPE"
            placeholder="文件类型"
            v-model:value="queryForm.fileType"
            width="200px"
         />
      </a-form-item>
      <a-form-item
...
import TemplateCommonForm from '/@/views/business/generate/template-common/template-common-form.vue';
...
async function queryData() {
  ...
  try {
    const queryResult = await templateCommonApi.queryPage(queryForm);
...
async function requestDelete(record) {
  SmartLoading.show();
  try {
    await templateCommonApi.delete(record.id);
...
async function requestBatchDelete() {
  SmartLoading.show();
    try {
      await templateCommonApi.batchDelete(selectedRowKeyList.value);
