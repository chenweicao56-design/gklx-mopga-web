...
-->
<template>
  <a-drawer title="编辑术语" :width="800" :open="visibleFlag" @close="onClose" :maskClosable="false" :destroyOnClose="true">
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 4 }">
      <a-form-item label="注释" name="databaseCommentTerm">
        <a-textarea v-model:value="form.databaseCommentTerm" placeholder="请输入注释" :rows="3" />
      </a-form-item>
...
<script setup>
  ...
  function loadDetail() {
    ...
    SmartLoading.show();
    databaseApi
      .getDatabaseTermDetail(databaseId.value)
      .then((res) => {
        const data = res.data || {};
        form.id = data.id;
  ...
  async function save() {
    ...
    try {
      await databaseApi.addOrUpdateDatabaseTerm({
        ...
        databaseCommentTerm: form.databaseCommentTerm === '' ? null : form.databaseCommentTerm,
      });
