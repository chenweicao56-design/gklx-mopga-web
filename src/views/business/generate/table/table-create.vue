...
-->
<template>
  <a-drawer style="z-index: 9999" title="快速建表" :width="1200" :open="visibleFlag" @close="onClose" :maskClosable="false" :destroyOnClose="true">
    ...
    <a-tabs v-model:activeKey="activeKey" @change="handleTabChange">
      ...
      <a-tab-pane key="2" tab="预览" force-render>
        <CusCodeMirror v-if="activeKey === '2'" ref="testCodeMirrorRef" v-model:model-value="previewCode" :height="'500px'" />
      </a-tab-pane>
    </a-tabs>
    <template #footer>
      <a-space>
...
<script setup>
  ...
  async function handleTabChange(key) {
    if (key === '2') {
      try {
        await formRef.value.validateFields();
        generateApi.createTablePreview(form).then((res) => {
  ...
  function startAi() {
    emitter.emit('ai-send', { agentAlias: 'sqlGenerateAgentService', data: JSON.stringify(form) });
  }

  onMounted(() => emitter.on('ai-listen', handleAiEvent));
