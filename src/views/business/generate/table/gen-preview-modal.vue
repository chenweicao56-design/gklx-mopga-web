    ...
    <a-tabs v-model:activeKey="fileKey" size="small" @change="onChangeTab">
      <a-tab-pane v-for="item in files" :key="item.fileName">
        <template #tab>
          ...
          <a-checkbox style="margin-left: 10px" v-model:checked="item.checked" @change="onCheckChange(item)" />
        </template>
      </a-tab-pane>
    </a-tabs>
    <div>
...
<script setup>
  ...
  import { previewStore } from '/@/store/modules/generate/preview';
  // ------------------ 显示，关闭 ------------------
  ...
  function syncSave() {
    Modal.confirm({
      ...
      onOk() {
        ...
        if (ft) {
          let files = ft.files.filter((item) => {
            return item.checked;
          });
