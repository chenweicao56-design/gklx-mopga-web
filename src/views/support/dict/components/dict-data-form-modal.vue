...
<template>
  <a-modal :open="visible" :title="form.dictDataId ? '编辑字典值' : '添加字典值'" ok-text="确认" cancel-text="取消" @ok="onSubmit" @cancel="onClose">
    ...
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
      ...
      <a-form-item label="扩展字段1" name="ext1">
        <a-input v-model:value="form.ext1" placeholder="请输入 扩展字段1" />
      </a-form-item>
      <a-form-item label="扩展字段2" name="ext2">
        <a-input v-model:value="form.ext2" placeholder="请输入 扩展字段2" />
      </a-form-item>
