<template>
  <div class="sql-design" @dragover.prevent @drop="handleDrop($event)">
    <div class="flex w-full flex-col h-full">
      <div class="flex-1 overflow-auto stage"></div>
    ...
    <TableDesignFilter ref="tableDesignFilterRef" @createTable="createTable" />
    <TableJoinPreview ref="tableJoinPreviewRef" />
  </div>
</template>
...
<script setup>
  ...
  import TableJoinPreview from '/@/components/business/design/table-join-preview.vue';
  import TableDesignFilter from '/@/components/business/design/table-design-filter.vue';
  ...
  async function handleDrop(e) {
    ...
    tableDesignFilterRef.value.show(table, e);
  }
  ...
  function addJoinLine({ table, targetTable, field, targetField }) {
    // 查找JOIN关系是否已存在
    let join = model.value.joins.find(
      (join) =>
        (join.table.id === table.id || join.table.id === targetTable.id) &&
        (join.targetTable.id === table.id || join.targetTable.id === targetTable.id)
    );
  ...
  function addAggregateLine({ table, targetTable, field, targetField }) {
    let aggregate = model.value.aggregates.find(
      (r) => r.table.id === table.id && r.targetTable.id === targetTable.id && (r.field.name === field.name || targetField.name === targetField.name)
    );
    if (aggregate == null) {
  ...
  function handlePreview() {
    console.log('handlePreview', model.value);
    if (_.isEmpty(model.value.tables)) {
      return;
    }

    tableJoinPreviewRef.value.show(props.databaseId, model.value);
  }

  const tableDesignFilterRef = ref();
  const tableJoinPreviewRef = ref();
