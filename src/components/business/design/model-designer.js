...
class ModelDesigner {
  ...
  constructor(container, whereOptions = [], width = 3500, height = 2000) {
    this.stageContainer = container;
    this.stageWidth = width;
    this.stageHeight = height;
    this.whereOptions = [{ dataLabel: '请选择', dataValue: '' }, ...whereOptions];
    this.#init();
  }
  ...
  createBackground() {
    ...
    // 正方形格子参数
    const cellSize = 20; // 每个正方形格子的边长
    const lineWidth = 1; // 线条宽度
    const lineColor = this.BACKGROUND_ITEM_COLOR; // 线条颜色

    const stageWidth = this.stage.width();
    const stageHeight = this.stage.height();

    // 计算水平和垂直方向能容纳的格子数量
    const horizontalCells = Math.ceil(stageWidth / cellSize);
    const verticalCells = Math.ceil(stageHeight / cellSize);

    // 计算偏移量，使网格在舞台中居中显示
    const xOffset = (stageWidth % cellSize) / 2;
    const yOffset = (stageHeight % cellSize) / 2;
    ...
    for (let i = 0; i <= horizontalCells; i++) {
      ...
      backgroundLayer.add(verticalLine);
    }
    ...
    for (let j = 0; j <= verticalCells; j++) {
      ...
      backgroundLayer.add(horizontalLine);
    }
  ...
  createTable(table, x, y, isChange = false) {
    ...
    const titleBackground = new Konva.Rect({
      ...
      fill: table.type === 'master' ? this.TABLE_TITLE_BACKGROUND_COLOR : this.SUB_TABLE_TITLE_BACKGROUND_COLOR,
      stroke: table.type === 'master' ? this.TABLE_TITLE_BACKGROUND_COLOR : this.SUB_TABLE_TITLE_BACKGROUND_COLOR,
      strokeWidth: 1,
    });
    // 创建标题
    ...
    for (let i = 0; i < table.fields.length; i++) {
      ...
      const fieldText2 = new Konva.Text({
        x: 25,
        y: 20,
        text: field.alias,
        fontSize: 10,
        fontFamily: 'Calibri',
        fill: this.GRAY_FONT_COLOR,
        height: this.TABLE_FIELD_HEIGHT,
        lineHeight: 2,
      });
      // 拖拽小球
      ...
      const selectBall = new Konva.Circle({
        name: 'a' + Math.round(Math.random() * 10000),
        x: this.TABLE_WIDTH - 25,
        y: 15,
        radius: 4,
        stroke: this.TABLE_FIELD_DRAG_BALL_BACKGROUND_COLOR,
        fill: field.selectChecked ? this.TABLE_BUTTON_BACKGROUND_COLOR : this.REVERSE_FONT_COLOR,
      });
      selectBall.on('click', function () {
        field.selectChecked = !field.selectChecked;
        if (field.selectChecked) {
          selectBall.fill(that.TABLE_BUTTON_BACKGROUND_COLOR);
        } else {
          selectBall.fill(that.REVERSE_FONT_COLOR);
        }
      });
      const whereBall = new Konva.Circle({
        name: 'a' + Math.round(Math.random() * 10000),
        x: this.TABLE_WIDTH - 10,
        y: 15,
        radius: 4,
        stroke: this.TABLE_FIELD_DRAG_BALL_BACKGROUND_COLOR,
        fill: field.whereType && field.whereType !== '' ? this.TABLE_BUTTON_BACKGROUND_COLOR : this.REVERSE_FONT_COLOR,
      });

      whereBall.on('click', function () {
        that.createSelectBox(whereBall, that.whereOptions, function (option) {
          field.whereType = option.dataValue;
          if (field.whereType === '') {
            whereBall.fill(that.REVERSE_FONT_COLOR);
            field.whereChecked = false;
          } else {
            whereBall.fill(that.TABLE_BUTTON_BACKGROUND_COLOR);
            field.whereChecked = true;
          }
        });
      });
      ...
      fieldGroup.add(fieldText2);
      fieldGroup.add(fieldDragBall);
      fieldGroup.add(selectBall);
      fieldGroup.add(whereBall);
      // 添加到表
  ...
  createLine = ({ lineType, field, targetField, table, targetTable, targetFieldBackgroundRect }) => {
    return new Promise((resolve, reject) => {
      try {
        ...
        const fieldGroup = tableGroup.findOne(`#${field.id}`);
        const targetFieldGroup = targetTableGroup.findOne(`#${targetField.id}`);
        // 不允许重复关联
        if (fieldGroup.__lines && fieldGroup.__lines.length > 0) {
          for (const line of fieldGroup.__lines) {
            // 完全重复，A.a1 => B.b1，再次操作为A.a1 => B.b1
            if (
              line.table === tableGroup &&
              line.targetTable === targetTableGroup &&
              line.field1 === fieldGroup &&
              line.field2 === targetFieldGroup
            ) {
              return reject(new Error('请勿重复关联！'));
            }
            // 反向重复，A.a1 => B.b1，再次操作为B.b1 => A.a1
            if (
              line.table === targetTableGroup &&
              line.targetTable === tableGroup &&
              line.field1 === targetFieldGroup &&
              line.field2 === fieldGroup
            ) {
              return reject(new Error('请勿重复关联！'));
            }
          }
  ...
  computeLinePoints({ field1, field2, table, targetTable }) {
    ...
    // 计算第一个坐标点
    const firstPoint = [0, leftFieldPosition.y + 15];
    ...
    turnPoint1 = [leftFieldPosition.x + this.TABLE_WIDTH + distance / 2, leftFieldPosition.y + 15];
    ...
    turnPoint4 = [leftFieldPosition.x + this.TABLE_WIDTH + distance / 2, rightFieldPosition.y + 15];
    ...
    // 计算最后一个坐标点
    const lastPoint = [0, rightFieldPosition.y + 15];
    ...
    if (
      lastPoint[0] > leftTable.absolutePosition().x &&
  ...
  createSelectBox(rect, options, event) {
    // 先移除已存在的选择框
    if (this.whereSelect) {
      this.whereSelect.destroy();
    }
    const pos = rect.getAbsolutePosition();

    // 创建选择框组
    const selectBox = new Konva.Group({
      x: pos.x + 8,
      y: pos.y,
      name: 'select-box',
    });
    ...
    // 选择框总高度
    const selectHeight = options.length * optionHeight;

    // 创建选择框背景
    const selectBackground = new Konva.Rect({
      width: selectWidth,
      height: selectHeight,
      fill: 'white',
      stroke: '#e5e7eb',
    ...
    selectBox.add(selectBackground);
    ...
    options.forEach((option, index) => {
      ...
      const optionText = new Konva.Text({
        x: 12,
        y: index * optionHeight + 8,
        text: option.dataLabel,
        fontSize: 14,
        fontFamily: 'Arial',
        fill: '#374151',
      });
      const whereTypeBall = new Konva.Circle({
        x: selectWidth - 20,
        y: index * optionHeight + 16,
        radius: 4,
        stroke: this.TABLE_FIELD_DRAG_BALL_BACKGROUND_COLOR,
        fill: option.whereType ? this.TABLE_BUTTON_BACKGROUND_COLOR : this.REVERSE_FONT_COLOR,
      });

      selectBox.add(optionBg);
      selectBox.add(optionText);
      selectBox.add(whereTypeBall);
      whereTypeBall.on('click', (e) => {
        e.cancelBubble = true;
        option.whereType = !option.whereType;
        whereTypeBall.fill(option.whereType ? this.TABLE_BUTTON_BACKGROUND_COLOR : this.REVERSE_FONT_COLOR);
      });

      [optionBg, optionText].forEach((element) => {
        // 选项鼠标悬停效果
        element.on('mouseover', () => {
          optionBg.fill('#f3f4f6');
        ...
        element.on('click', (e) => {
          ...
          selectBox.destroy();
          this.whereSelect = null;
          this.elementLayer.draw();
        });
      });

      // 使选项可点击
      optionBg.listening(true);
      optionText.listening(true);
    });

    // 将选择框添加到层
    this.elementLayer.add(selectBox);
    this.whereSelect = selectBox;

    // 定位选择框
    // positionSelectBox(rect);
